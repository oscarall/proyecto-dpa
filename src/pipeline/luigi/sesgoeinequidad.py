...
from src.pipeline.luigi.sesgoeinequidad_metadata import SesgoeInequidadMetadata
from src.pipeline.entrenamiento import run_magic_loop //quitar con version final
from src.utils.constants import ENTRENAMIENTO_PATH, BUCKET //revisar esto con datos finales
from src.pipeline.ingesta_almacenamiento import get_s3_resource //quitar con final
from src.utils.task import DPATask

class Sesgoeinequidad(DPATask):
    ingesta = luigi.Parameter(default="consecutiva")
    date = luigi.Parameter(default=None)

    def requires(self):
        return SesgoeInequidadMetadata(ingesta=self.ingesta, date=self.date)

    def run(self):
        data = None
        metadata = {
            "id": self.task_id,
            "step": 8 //revisar esto de steps
        }
    ...
    def output(self):
        ...
        return luigi.contrib.s3.S3Target(
            ...
            os.path.join(tempfile.gettempdir(), "sesgoeinequidad", f"sesgoeinequidad-metadata-{self.ingesta}-{self.date}.json")
        )